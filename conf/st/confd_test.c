...
int main(int argc, char *argv[])
{
	...
	rc = M0_GETOPTS("confd_test", argc, argv,
	...
	n = m0_confx_db_read(dbpath, &db_obj);
	if (n <= 0) {
		rc = n;
		goto cleanup;
	}
	...
	for (i = 0; i < n; ++i) {
		rc = m0_conf_obj_find(&reg, db_obj[i].o_conf.u_type,
				      &db_obj[i].o_id, &obj);
		if (rc != 0)
			break;

		rc = m0_conf_obj_fill(obj, &db_obj[i], &reg);
		if (rc != 0)
			break;
	}
