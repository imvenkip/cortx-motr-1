...
#include "motr/magic.h"         /* M0_HALON_INTERFACE_MAGIC */
	...
	for (i = 0; i < nvec->hmnv_nr; ++i) {
		...
		if (m0_fid_tget(&note->no_id) ==
			...
			if (note->no_state == hii->hii_nvec[j].no_state &&
				...
				M0_LOG(M0_DEBUG, "ignoring the same state for "
						...
						struct m0_thread *thread)
{
	...
	motr = &hi->hif_internal->hii_instance;
	rc = m0_thread_adopt(thread, motr);
