...
#ifndef __COLIBRI_LIB_USER_SPACE_GETOPTS_H__
#define __COLIBRI_LIB_USER_SPACE_GETOPTS_H__
...
enum c2_getopts_opt_type {
	...
	GOT_NUMBER,
	/**
	   An option with a numerical argument, followed by a multiplier suffix.
	   The argument is expected in the format that c2_get_bcount() can
	   parse.  When this option is encountered, its call-back
	   c2_getopts_opt::go_u::got_scaled() is invoked with the parsed
	   argument as its sole parameter.
	 */
	GOT_SCALED,
	/**
	   An option with a string argument. When this option is encountered,
	   its call-back c2_getopts_opt::go_u::got_string() is invoked with the
...
struct c2_getopts_opt {
	...
	union c2_getopts_union {
		...
		void   (*got_scaled)(c2_bcount_t num);
 ...
 */
#define C2_SCALEDARG(ch, desc, func) {		\
	.go_type = GOT_SCALED,			\
	.go_opt  = (ch),			\
	.go_desc = (desc),			\
...
/* __COLIBRI_LIB_USER_SPACE_GETOPTS_H__ */
#endif
