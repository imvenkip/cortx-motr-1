...
#define M0_FORMAT_FOOTER_UPDATE(n) \
	...
	m0_format_footer_update(n);				\
	M0_LOG(M0_ALWAYS, "node=%p new=%"PRIx64, (n), (n)->b_footer.ft_checksum); })

#define M0_FORMAT_FOOTER_UPDATE_T(t) \
	({ M0_LOG(M0_ALWAYS, "tree=%p old=%"PRIx64, (t), (t)->bb_footer.ft_checksum); \
...
static m0_bcount_t be_btree_ksize(struct m0_be_btree *btree,
				  struct m0_be_seg *seg,
				  const void *key)
{
	int rc;
	...
	/* XXX: size, seg! */
	m0_be_reg_get(&M0_BE_REG(seg, 32, (void *)key), NULL);

	rc = ops->ko_ksize(key);

	/* XXX: size, seg! */
	m0_be_reg_put(&M0_BE_REG(seg, 32, (void *)key), NULL);

	return rc;
}

static m0_bcount_t be_btree_vsize(struct m0_be_btree *btree,
				  struct m0_be_seg *seg,
				  const void *data)
{
	int rc;
					...
					bool root)
{
	...
	r1 = _0C(ergo(node && node->b_header.hd_magic,
		      m0_format_footer_verify(node) == 0));

	r2 = _0C(node->b_level <= BTREE_HEIGHT_MAX);

	r3 = _0C(ergo(root, 0 <= node->b_nr_active &&
		      node->b_nr_active <= KV_NR));

	r4 = _0C(ergo(!root, BTREE_FAN_OUT-1 <= node->b_nr_active &&
		      node->b_nr_active <= KV_NR));

	r5 = _0C(ergo(node->b_nr_active > 1,
			      ...
			      struct m0_be_tx    *tx)
{
	...
	if (node->b_nr_active > 0) {
		mem_update(btree, seg, tx, node->b_key_vals,
			   sizeof(*node->b_key_vals) * KV_NR /* node->b_nr_active */);
		mem_update(btree, seg, tx, node->b_children,
			   sizeof(*node->b_children) * (/* node->b_nr_active */ KV_NR + 1));
	}
...
del_loop:
	for (i = 0;; i = 0) {
		...
		M0_BE_REG_GET_PTR(rsibling, seg, tx);
		M0_BE_REG_GET_PTR(lsibling, seg, tx);
		{
		...
		if (node_b_nr_active == BTREE_FAN_OUT - 1 && parent) {
			if (rsibling &&
		...
		M0_BE_REG_PUT_PTR(lsibling, seg, tx);
		M0_BE_REG_PUT_PTR(rsibling, seg, tx);
	}
	...
	if (node_b_children_index_b_nr_active > BTREE_FAN_OUT - 1) {
		...
		M0_BE_REG_GET_PTR(child.p_node, seg, tx);
		...
		mem_update(btree, seg, tx, &child.p_node->b_footer,
			   sizeof(child.p_node->b_footer));
		mem_update(btree, seg, tx, &node->b_key_vals[index],
			   sizeof(node->b_key_vals[0]));
		mem_update(btree, seg, tx,
			   &child.p_node->b_key_vals[child.p_index],
			   sizeof(child.p_node->b_key_vals[0]));
		M0_BE_REG_PUT_PTR(child.p_node, seg, tx);
		M0_BE_REG_PUT_PTR(node, seg, tx);
		       ...
		       uint64_t                   zonemask)
{
	...
	if ((cur_kv == NULL && optype != BTREE_SAVE_UPDATE) ||
		...
		if (cur_kv != NULL && optype != BTREE_SAVE_INSERT) {
			if (vsz > be_btree_vsize(tree, seg, cur_kv->val)) {
				/*
			  ...
			  struct m0_be_tx_credit   *accum)
{
	...
	btree_rebalance_credit(tree, seg, &cred);
	m0_be_tx_credit_mac(accum, &cred, nr);
}
				    ...
				    struct m0_buf *dest_value)
{
	...
	if (kv != NULL) {
		...
		vsize = be_btree_vsize(tree, seg, kv->val);
				    ...
				    struct m0_buf *out)
{
	...
	m0_buf_init(out, key, key == NULL ? 0 : be_btree_ksize(tree, seg, key));
					  ...
					  uint64_t                   zonemask)
{
	...
	M0_PRE(tree->bb_root != NULL && tree->bb_ops != NULL);
	M0_PRE(key->b_nob == be_btree_ksize(tree, NULL /* XXX: seg */,
					    key->b_addr));
					...
					const struct m0_buf *key, bool slant)
{
	...
	last = get_btree_node(cur, key->b_addr, slant, seg);
	...
	if (last.p_node == NULL) {
		...
		M0_BE_REG_GET_PTR(cur->bc_node, seg, NULL);
...
static int btree_cursor_seek(struct m0_be_btree_cursor *cur, void *key)
{
	const struct m0_buf kbuf =
		M0_BUF_INIT(be_btree_ksize(cur->bc_tree, cur->bc_seg, key), key);
...
M0_INTERNAL void m0_be_btree_cursor_next(struct m0_be_btree_cursor *cur)
{
	...
	if (node_b_leaf) {
		...
		while (node && cur->bc_pos >= node_b_nr_active) {
