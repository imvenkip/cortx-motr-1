			   ...
			   struct c2_pdclust_layout **pl)
{
	...
	C2_UT_ASSERT(rc == 0);
	//todo C2_UT_ASSERT(c2_layout_find(&domain, lid) == &(*pl)->pl_base.sl_base);
...
static uint32_t nr_assign(uint32_t inline_test, uint32_t less, uint32_t more)
{
	uint32_t nr;
			      ...
			      uint32_t inline_test)
{
	...
	if (enum_id == LIST_ENUM_ID) {
		nr = nr_assign(inline_test, 9, 109);
		A = A_NONE;
		B = B_NONE;
	...
	//todo C2_UT_ASSERT(c2_layout_find(&domain, lid) == NULL);
			       ...
			       uint32_t inline_test)
{
	...
	if (enum_id == LIST_ENUM_ID) {
		nr = nr_assign(inline_test, 5, 125);
		A = A_NONE;
		B = B_NONE;
	} else {
		nr = 1500;
	...
	C2_UT_ASSERT(rc == 0);
	//todo C2_UT_ASSERT(c2_layout_find(&domain, lid) == l);
			       ...
			       uint32_t inline_test)
{
	...
	if (enum_id == LIST_ENUM_ID) {
		nr = nr_assign(inline_test, 10, 120);
		A = A_NONE;
		B = B_NONE;
				      ...
				      uint32_t inline_test)
{
	...
	if (enum_id == LIST_ENUM_ID) {
		nr = nr_assign(inline_test, 3, 103);
				      ...
				      uint32_t inline_test)
{
	...
	if (enum_id == LIST_ENUM_ID) {
		nr = nr_assign(inline_test, 13, 113);
		rc = pdclust_layout_build(LIST_ENUM_ID, lid,
					  45, 5, 4096, &seed,
					  nr, A_NONE, B_NONE,
					  &pl, &list_enum, NULL);
				 ...
				 uint32_t inline_test)
{
	...
	if (enum_id == LIST_ENUM_ID) {
		nr = nr_assign(inline_test, 14, 1014);
		rc = pdclust_layout_build(LIST_ENUM_ID, lid,
					  100, 10, 4096, &seed,
					  nr, A_NONE, B_NONE,
					  &pl, &list_enum, NULL);
	...
	enum_op_verify(enum_id, lid, nr, &pl->pl_base.sl_base);
			       ...
			       bool failure_test)
{
	...
	if (existing_test && !failure_test) {
		//todo C2_UT_ASSERT(c2_layout_find(&domain, lid) == l2);
			    ...
			    bool layout_destroy, struct c2_layout **l_obj)
{
	...
	/* Build a layout object. */
	if (enum_id == LIST_ENUM_ID) {
		nr = nr_assign(inline_test, 7, 1900);
		rc = pdclust_layout_build(LIST_ENUM_ID, lid,
					  5, 2, 4096, &seed,
					  nr, A_NONE, B_NONE,
					  &pl, &list_enum, NULL);
			       ...
			       uint32_t inline_test)
{
	...
	if (existing_test) {
		...
		if (enum_id == LIST_ENUM_ID) {
			nr = nr_assign(inline_test, 13, 123);
			rc = pdclust_layout_build(LIST_ENUM_ID, lid,
						  4, 1, 4096, &seed,
						  nr, A_NONE, B_NONE,
						  &pl, &list_enum, NULL);
			       ...
			       uint32_t inline_test)
{
	...
	if (existing_test) {
		...
		if (enum_id == LIST_ENUM_ID) {
			nr = nr_assign(inline_test, 12, 122);
			rc = pdclust_layout_build(LIST_ENUM_ID, lid,
						  4, 1, 4096, &seed,
						  nr, A_NONE, B_NONE,
						  &pl, &list_enum, NULL);
