...
static const struct m0_addb_ctx_type layout_addb_ctx_type = {
	.act_name = "layout"
};

const struct m0_addb_loc layout_addb_loc = {
	.al_name = "layout"
};

struct m0_addb_ctx layout_global_ctx = {
	.ac_type   = &layout_addb_ctx_type,
	.ac_parent = &m0_addb_global_ctx
};

M0_ADDB_EV_DEFINE(layout_decode_fail, "layout_decode_fail",
		  M0_ADDB_EVENT_LAYOUT_DECODE_FAIL, M0_ADDB_CALL);
M0_ADDB_EV_DEFINE(layout_encode_fail, "layout_encode_fail",
		  M0_ADDB_EVENT_LAYOUT_ENCODE_FAIL, M0_ADDB_CALL);

M0_ADDB_EV_DEFINE(layout_lookup_fail, "layout_lookup_fail",
		  M0_ADDB_EVENT_LAYOUT_LOOKUP_FAIL, M0_ADDB_CALL);
M0_ADDB_EV_DEFINE(layout_add_fail, "layout_add_fail",
		  M0_ADDB_EVENT_LAYOUT_ADD_FAIL, M0_ADDB_CALL);
M0_ADDB_EV_DEFINE(layout_update_fail, "layout_update_fail",
		  M0_ADDB_EVENT_LAYOUT_UPDATE_FAIL, M0_ADDB_CALL);
				 ...
				 const struct m0_layout_ops *ops)
{
	...
	m0_addb_ctx_init(&l->l_addb, &layout_addb_ctx_type, &layout_global_ctx);
 ...
 */
static void addb_add(struct m0_addb_ctx *ctx,
		     const struct m0_addb_ev *ev,
		     const char *err_msg,
		     int rc)
{

	M0_PRE(M0_IN(ev->ae_id, (layout_decode_fail.ae_id,
	...
	switch (ev->ae_id) {
	case M0_ADDB_EVENT_FUNC_FAIL:
		M0_ADDB_ADD(ctx, &layout_addb_loc, m0_addb_func_fail,
			    err_msg, rc);
		break;
	case M0_ADDB_EVENT_OOM:
		M0_ADDB_ADD(ctx, &layout_addb_loc, m0_addb_oom);
		break;
	case M0_ADDB_EVENT_LAYOUT_DECODE_FAIL:
		M0_ADDB_ADD(ctx, &layout_addb_loc, layout_decode_fail, rc);
		break;
	case M0_ADDB_EVENT_LAYOUT_ENCODE_FAIL:
		M0_ADDB_ADD(ctx, &layout_addb_loc, layout_encode_fail, rc);
		break;
	case M0_ADDB_EVENT_LAYOUT_LOOKUP_FAIL:
		M0_ADDB_ADD(ctx, &layout_addb_loc, layout_lookup_fail, rc);
		break;
	case M0_ADDB_EVENT_LAYOUT_ADD_FAIL:
		M0_ADDB_ADD(ctx, &layout_addb_loc, layout_add_fail, rc);
		break;
	case M0_ADDB_EVENT_LAYOUT_UPDATE_FAIL:
		M0_ADDB_ADD(ctx, &layout_addb_loc, layout_update_fail, rc);
		break;
	case M0_ADDB_EVENT_LAYOUT_DELETE_FAIL:
		M0_ADDB_ADD(ctx, &layout_addb_loc, layout_delete_fail, rc);
		break;
	/* default: M0_ASSERT(0); This is covered by the M0_PRE() above. */
	}
}
