...
/*
 * Converts lustre fid to mero fid.
 */
static void lu_fid_2_m0_fid(const lustre_fid *fid, struct m0_fid *mfid)
{
	M0_CASSERT(sizeof fid->f_seq == 8);
	M0_CASSERT(sizeof fid->f_oid == 4);
	M0_CASSERT(sizeof fid->f_ver == 4);

	if (fid->f_ver != 0)
		CT_ERROR(-EINVAL, "invalid FID:"DFID, PFID(fid));

	mfid->f_container = fid->f_seq >> 8;
	mfid->f_key       = fid->f_oid << (32 - 8) |
			    (fid->f_seq & 0xff) << (64 - 8);
}
			   ...
			   const lustre_fid *fid)
{
	...
	lu_fid_2_m0_fid(fid, &mfid);
	return snprintf(buf, sz, "%s/"LPX64":"LPX64, archive_dir, mfid.f_container,
			mfid.f_key);
}
...
static int ct_archive(const struct hsm_action_item *hai, const long hal_flags)
{
	...
	if (hai->hai_extent.length == -1) {
		/* XXX: MERO does not support rename operation at the moment. */
