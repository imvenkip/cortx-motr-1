...
/**
 * @defgroup kc2ctl Colibri Kernel-space Control
 *
 * @brief kc2ctl driver provides a debugfs interface to control kcolibri in
 * runtime. All control files are placed under "colibri/" directory in the root
 ...
 *
 * This file is responsible only for creation and cleanup of colibri/ directory
...
const char     dfs_root_name[] = "colibri";

int dfs_init(void)
{
	int rc;

	/* create colibri's main debugfs directory */
...
void dfs_cleanup(void)
{
	...
	/*
	 * remove all orphaned debugfs files (if any) and colibri's debugfs root
	 * directroy itself
	 */
	if (dfs_root_dir != 0) {
		debugfs_remove_recursive(dfs_root_dir);
...
int __init c2ctl_init(void)
{
	return dfs_init();
}

void __exit c2ctl_exit(void)
{
	dfs_cleanup();
}

module_init(c2ctl_init);
module_exit(c2ctl_exit);

MODULE_AUTHOR("Xyratex International");
MODULE_DESCRIPTION("Colibri control interface");
MODULE_LICENSE("GPL");
