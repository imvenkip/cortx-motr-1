...
unload_kernel_module()
{
        ...
        if [ $? -ne "0" ]
        then
                echo "Failed:failed to remove existing module \
                      $colibri_module ."
                return 1
        fi
}
...
pool_width=$pool_width,nr_data_units=$data_units,nr_parity_units=$parity_units \
        ...
        echo "Successfully written data of specified size and count" \
             "to c2t1fs file."
        ...
        echo "Successfully read data of specified size and count" \
             "from c2t1fs file."
...
io_combinations()
{
        # This test run for different different stripe_size values
