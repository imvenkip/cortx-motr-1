...
umount_c2t1fs()
{
        ...
        if [ $? -ne "0" ]
        then
                echo "Failed to  unmount c2t1fs file system."
                return 1
        fi

        rm -f $c2t1fs_mount_dir
...
io_combinations()
{
        ...
        do
            ...
            mount_c2t1fs $strip_size $pool_width $data_units $parity_units
            if [ $? -ne "0" ] 
            then
                    echo "Failed : failed to mount c2t1fs."
                    return 1
            fi
            ...
            do
                ...
                bulkio_test $io_size 1 &>> $COLIBRI_TEST_LOGFILE
                if [ $? -ne "0" ]
                then
                        umount_c2t1fs
                        return 1
            ...
            if [ $? -ne "0" ] 
            then
                    echo "Failed : failed to umount c2t1fs."
                    return 1
            fi
...
main()
{
        ...
        if [ $? -ne "0" ]
        then
                echo "Failed : Module unload failed."
        fi
