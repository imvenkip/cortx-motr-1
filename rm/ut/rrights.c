...
static void rm_disconnect(struct rm_context *src, const struct rm_context *dest)
{
	...
	rc = m0_rpc_session_destroy(&src->rc_sess[dest->rc_id], 30);
	C2_UT_ASSERT(rc == 0);

	rc = m0_rpc_conn_destroy(&src->rc_conn[dest->rc_id], 30);
	C2_UT_ASSERT(rc == 0);
			...
			int value)
{
	...
	m0_rm_incoming_init(in, owner, C2_RIT_LOCAL, RIP_NONE, flag);
			     ...
			     enum rm_server dsrv_id)
{
	...
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&owner->ro_sublet));
...
static void test2_verify()
{
	...
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&so2->ro_sublet));
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&so2->ro_borrowed));
	C2_UT_ASSERT(m0_rm_ur_tlist_is_empty(&so2->ro_owned[OWOS_CACHED]));
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&so1->ro_borrowed));
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&so1->ro_owned[OWOS_CACHED]));
}
...
static void test3_verify()
{
	...
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&so3->ro_sublet));
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&so2->ro_borrowed));
	C2_UT_ASSERT(!m0_rm_ur_tlist_is_empty(&so2->ro_owned[OWOS_CACHED]));
	C2_UT_ASSERT(m0_rm_ur_tlist_is_empty(&so1->ro_borrowed));
	C2_UT_ASSERT(m0_rm_ur_tlist_is_empty(&so1->ro_owned[OWOS_CACHED]));
