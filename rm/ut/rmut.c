...
static void rm_init(void)
{
	...
	Sauron.ro_sm.sm_state = ROS_FINAL;
	...
	elves.ro_sm.sm_state = ROS_FINAL;
	c2_rm_owner_init(&elves, &R.rs_resource, NULL);
	dwarves.ro_sm.sm_state = ROS_FINAL;
	c2_rm_owner_init(&dwarves, &R.rs_resource, NULL);
	men.ro_sm.sm_state = ROS_FINAL;
...
{
	in.rin_sm.sm_state = RI_SUCCESS;
	inreq.rin_sm.sm_state = RI_SUCCESS;
	inrep.rin_sm.sm_state = RI_SUCCESS;
	inother.rin_sm.sm_state = RI_SUCCESS;
	inconflict.rin_sm.sm_state = RI_SUCCESS;
}
...
static void right_get_test0(void)
{
	...
	in.rin_sm.sm_state = RI_INITIALISED;
	in.rin_priority = 0;
	...
	//C2_ASSERT(result == 0);
	C2_ASSERT(in.rin_sm.sm_state == RI_SUCCESS);
...
static void right_get_test1(void)
{
	...
	inother.rin_sm.sm_state = RI_INITIALISED;
...
static void intent_client(int id)
{
	...
	/* Gets the right from the server */
        ...
        req->in.rin_sm.sm_state = RI_SUCCESS;
...
static void wbc_server(int id)
{
	...
	/* Gets the loan request from client */
        ...
        req->in.rin_sm.sm_state = RI_INITIALISED;
        req->in.rin_priority = 0;
        req->in.rin_ops = &rings_incoming_ops;
        req->in.rin_want.ri_ops = &rings_right_ops;
        req->in.rin_type = C2_RIT_LOCAL;
...
static void cancel_server(int id)
{
	...
	/* witness SERVER possesses right DURIN. */
        ...
        inreq.rin_sm.sm_state = RI_INITIALISED;
...
static void caching_client(int id)
{
	...
	//c2_chan_init(&inrep.rin_signal);
        ...
        inrep.rin_sm.sm_state = RI_INITIALISED;
...
static void callback_client1(int id)
{
	...
	//c2_chan_init(&inconflict.rin_signal);
        ...
        inconflict.rin_sm.sm_state = RI_INITIALISED;
