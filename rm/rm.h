...
/**
   ...
   @todo Remove after generic cookie implementation.
 */
struct c2_cookie {
...
struct c2_rm_remote {
	...
	this field? Alternately, we have to replace this field with connection
	or session pointer.
...
enum c2_rm_incoming_flags {
	...
	/**
	   ...
	   the following:
		 ...
		 is typical for a local request (C2_RIT_LOCAL), because local users
		 ...
		 wanted right. This is typical for a remote request (C2_RIT_BORROW
		 or C2_RIT_REVOKE);
...
/**
  Copies rights data into a raw buffer.
 */
int c2_rm_rdatum2buf(struct c2_rm_right *right, void **buf,
		     c2_bcount_t *bytesnr);

/**
  Copies raw buffer in rights data (datum).
 */
