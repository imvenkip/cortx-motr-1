...
/**
   ...
   - An general purpose Event Queue (EQ).  This is represented by the
     nlx_kcore_transfer_mc::ktm_eqh handle.  It is used for all operations
     except @c LNetGet().
   - A dedicated Event Queue (EQ) for @c LNetGet() operations.  This is
     represented by the nlx_kcore_transfer_mc::ktm_LNetGet_eqh.

   The nlx_core_tm_start() subroutine creates the event handles.  The
   nlx_core_tm_stop() subroutine releases the handles.
   ...
   All MDs are associated with the general EQ of the transfer machine
   (nlx_kcore_transfer_mc::ktm_eqh), unless they are used by an @c LNetGet()
   operation, in which case they will use the EQ dedicated for this purpose
   (nlx_kcore_transfer_mc::ktm_LNetGet_eqh).
   ...
   with a single producer and single consumer resulted in the initial decision
   to use just one LNet EQ per transfer machine
   (nlx_kcore_transfer_mc::ktm_eqh).

   However, subsequent investigation determined that we need a separate EQ to
   safely distinguish between @c LNET_EVENT_SEND events generated by @c
   LNetPut() and @c LNetGet().  We need to handle such events for @c LNetPut()
   operations but ignore them for @c LNetGet() operations.  It would have been
   possible to share a single EQ, except that LNet does not guarantee the order
   of the SEND/REPLY events generated by @c LNetGet().  If the SEND follows the
   REPLY, then we would have already deleted or modified contents of our
   private data associated with the event - the very same data that needs to be
   de-referenced to determine if a SEND should be ignored or not.  The only
   safe way to handle this is to use a separate EQ with a separate callback
   that can blindly ignore the SEND events without needing to de-reference the
   user private data of the event.

   Both EQs are created in the call to the nlx_core_tm_start() subroutine, and
   are freed in the call to the nlx_core_tm_stop() subroutine.
   ...
   -# It will ignore @c LNET_EVENT_SEND events delivered as a result of a @c
      LNetGet() call.  This is actually handled by the event handler dedicated
      to the MDs used in this call, for reasons mentioned above.  This separate
      handler falls through into the main callback subroutine for the remaining
      cases.
   -# It will ensure that @c LNET_EVENT_REPLY events are always processed as
      though they were second in the sequence of SEND/REPLY events.  LNet does
      not guarantee this order, but the separate handler can tweak the unlinked
      field of a REPLY event before calling the main handler to ensure
      proper behavior.
   ...
   -# Create an MD using @c LNetMDBind() with each invocation of the
      ...
      Save the MD handle in the nlx_kcore_buffer::kb_mdh field.
      Set up the fields of the @c lnet_md_t argument as follows:
      - Set the @c eq_handle to identify the general purpose EQ associated with
        the transfer machine (nlx_kcore_transfer_mc::ktm_eqh) for @c LNetPut()
        operations, or the dedicated EQ for @c LNetGet() operations
	(nlx_kcore_transfer_mc::ktm_LNetGet_eqh).
      - Set the kernel logical address of the nlx_core_buffer in the
        @c user_ptr field.
      - Pass in the KIOV from the nlx_kcore_buffer::kb_kiov.
   ...
   -# The kernel Core module registers two callback subroutines with the
      LNet EQs defined per transfer machine. LNet requires that these subroutines
...
/**
   Dedicated callback for LNetGet operation events.
   It filters out SEND events, and ensures that REPLY events have their
   unlinked value set. It invokes the general callback to actually handle
   the event.
 */
static void nlx_kcore_LNetGet_eq_cb(lnet_event_t *event)
{
	bool forced_unlinked = false;

	C2_PRE(event != NULL);
	if (event->type == LNET_EVENT_SEND) {
#ifdef NLX_DEBUG
		NLXDBGP(&nlx_debug, 1, "nlx_kcore_eq_cb: filtered SEND");
		NLXDBG(&nlx_debug, 2,
		       nlx_kprint_lnet_event("nlx_kcore_eq_cb", event));
#endif
		return;
	}
	if (event->type == LNET_EVENT_REPLY && event->unlinked == 0) {
		forced_unlinked = true;
		event->unlinked = 1;
#ifdef NLX_DEBUG
		NLXDBGP(&nlx_debug, 1, "nlx_kcore_eq_cb: REPLY forced unlink");
#endif
	}
	nlx_kcore_eq_cb(event);
	if (forced_unlinked)
		event->unlinked = 0;
	return;
}
		      ...
		      struct c2_net_end_point **epp)
{
	...
	if (rc < 0)
		goto fail;
	rc = LNetEQAlloc(C2_NET_LNET_EQ_SIZE, nlx_kcore_LNetGet_eq_cb,
...
fail_with_eq:
	i = LNetEQFree(kctm->ktm_eqh);
	C2_ASSERT(i == 0);
	i = LNetEQFree(kctm->ktm_LNetGet_eqh);
	C2_ASSERT(i == 0);
fail:
	c2_free(e1);
	c2_free(e2);
...
void nlx_core_tm_stop(struct nlx_core_transfer_mc *lctm)
{
	...
	C2_ASSERT(rc == 0);
	rc = LNetEQFree(kctm->ktm_LNetGet_eqh);
	C2_ASSERT(rc == 0);
