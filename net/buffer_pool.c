...
static bool pool_colour_check(const struct c2_net_buffer_pool *pool)
{
	...
	for (i = 0; i < pool->nbp_colours_nr; i++) {
		c2_tlist_for(&c2_net_tm_tl, &pool->nbp_colour[i], nb) {
...
{
	struct c2_net_buffer *nb;
	if (!c2_net_pool_tlist_is_empty(&pool->nbp_lru)) {
		c2_tlist_for(&c2_net_pool_tl, &pool->nbp_lru, nb) {
			    ...
			    uint32_t colours)
{
	...
	for (i = 0; i < colours; i++)
		c2_net_tm_tlist_init(&pool->nbp_colour[i]);
}
...
{
	c2_net_pool_tlink_del_fini(nb);
	if (c2_net_tm_tlink_is_in(nb))
		c2_net_tm_tlist_del(nb);
	c2_net_tm_tlink_fini(nb);
...
void c2_net_buffer_pool_fini(struct c2_net_buffer_pool *pool)
{
	...
	for (i = 0; i < pool->nbp_colours_nr; i++)
		c2_net_tm_tlist_fini(&pool->nbp_colour[i]);
					     ...
					     uint32_t colour)
{
	...
	if (pool->nbp_free <= 0)
		return NULL;
	if (colour != ~0 &&
	    !c2_net_tm_tlist_is_empty(&pool->nbp_colour[colour]))
		nb = c2_net_tm_tlist_head(&pool->nbp_colour[colour]);
	else
		nb = c2_net_pool_tlist_head(&pool->nbp_lru);
	C2_ASSERT(nb != NULL);
	c2_net_pool_tlist_del(nb);
	if (c2_net_tm_tlink_is_in(nb))
		c2_net_tm_tlist_del(nb);
	C2_CNT_DEC(pool->nbp_free);
			    ...
			    struct c2_net_buffer *buf, uint32_t colour)
{
	...
	if (colour != ~0) {
		...
		c2_net_tm_tlist_add(&pool->nbp_colour[colour], buf);
	}
	c2_net_pool_tlist_add_tail(&pool->nbp_lru, buf);
