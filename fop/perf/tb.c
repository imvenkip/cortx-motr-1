...
static int ub_fom_mutex_per_lock_tick(struct m0_fom *fom);
...
static int (*ticks[UB_FOM_NR])(struct m0_fom *fom) = {
	...
	[UB_FOM_MUTEX_PER_LOC] = ub_fom_mutex_per_lock_tick,
...
static int ub_fom_mem_tick(struct m0_fom *fom)
{
	...
	cpu_utilize(ctx, rq_seqn, rq_type, rq_type == UB_FOM_MEM_B ?
		    UB_FOM_BYTE_CYCLES : UB_FOM_CYCLES);
...
/* ub_fom_mutex_per_lock_tick() function is written respectively to show how
   performance is decreased in case when mutex synchronisation is used and
   mutex is not shared */
static int ub_fom_mutex_per_lock_tick(struct m0_fom *fom)
{
	struct ub_fom_ctx *ctx;
	...
	ctx = container_of(fom, struct ub_fom_ctx, fc_gen);
	rq_seqn = ctx->fc_seqn;
	lock_index = ctx->fc_seqn % ctx->fc_lock_nr;

	m0_mutex_lock(&ctx->fc_lock[lock_index]);

	M0_LOG(M0_DEBUG, "fom:seqn=%zu", rq_seqn);
	cpu_utilize(ctx, rq_seqn, UB_FOM_MEM_KB, UB_FOM_CYCLES);

	m0_mutex_unlock(&ctx->fc_lock[lock_index]);
	m0_fom_phase_set(fom, M0_FOM_PHASE_FINISH);
...
static int ub_fom_long_tick(struct m0_fom *fom)
{
	...
	/* Every nth lock is a write lock, the rest are read-locks. */
	is_writer = rq_seqn % UB_FOM_WRITER_OCCURRENCE == 0;
			  ...
			  enum ub_fom_type test_type)
{
	...
	for (i = 0; i < UB_FOM_COUNT; ++i)
		req_handle(reqh[i % reqh_nr], i, test_type);

	for (i = 0; i < reqh_nr; ++i)
		m0_reqh_fom_domain_idle_wait(reqh[i]);
}
