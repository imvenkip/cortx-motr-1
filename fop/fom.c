...
static void loc_handler_thread(struct c2_fom_hthread *th)
{
	...
	do {
		...
		idle = fom == NULL;
	         ...
	         loc->fl_idle_threads_nr <= loc->fl_lo_idle_threads_nr);
...
{
	C2_PRE(cb->fc_fom != NULL);
	C2_PRE(cb->fc_fom->fo_state == C2_FOS_RUNNING);
