		...
		case CTG_OP_COMBINE(CO_PUT, CT_META):
			*(uint64_t *)arena = sizeof(struct m0_cas_ctg *);
			...
			if (rc == 0)
				m0_chan_broadcast_lock(ctg_chan);
			if(temp_fid)
			M0_LOG(M0_ALWAYS,"vcp: ctg = %p,ct = %d, opc = %d"FID_F,*(struct m0_cas_ctg **)(arena + M0_CAS_CTG_KV_HDR_SIZE),ct,opc,FID_P(temp_fid));
			break;
	...
	if (ctg_op->co_opcode != CO_GC &&
		...
		ctg_op->co_rc = ctg_buf_get(&ctg_op->co_key,
					    &M0_BUF_INIT_CONST(
						    sizeof(struct m0_fid),
						    fid), true);
	M0_LOG(M0_ALWAYS,"vcp:meta = %p,opcode = %d, "FID_F,m0_ctg_meta(),ctg_op->co_opcode, FID_P(fid));
	if(ctg_op->co_fom)
	M0_LOG(M0_ALWAYS,"vcp: tx = %p ",&ctg_op->co_fom->fo_tx.tx_betx);
	if (ctg_op->co_rc != 0) {
		ret = M0_FSO_AGAIN;
		    ...
		    int                  next_phase)
{
	...
	M0_LOG(M0_ALWAYS,"vcp ctg = %p, meta_ctg = %p, cctg_ctg = %p, opcode = %d,", ctg, m0_ctg_meta(), m0_ctg_ctidx(),ctg_op->co_opcode);
	if(ctg_op->co_fom)
	M0_LOG(M0_ALWAYS,"vcp: tx = %p", &ctg_op->co_fom->fo_tx.tx_betx);
