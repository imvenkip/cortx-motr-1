...
static void ctg_store_init_creds_calc(struct m0_be_seg       *seg,
				      struct m0_cas_ctg      *meta,
				      struct m0_cas_ctg      *ctidx,
				      struct m0_be_tx_credit *cred);

static int  ctg_meta_create  (struct m0_be_seg  *seg,
			      struct m0_be_tx   *tx,
			      struct m0_cas_ctg **meta);
static void ctg_meta_destroy (struct m0_cas_ctg *meta,
			   ...
			   struct m0_be_tx    *tx)
{
	...
	key = M0_BUF_INIT_PTR(&key_data);
	anchor.ba_value.b_nob = sizeof(uint64_t) + sizeof(struct m0_cas_ctg);
	...
	if (rc == 0) {
		...
		*(uint64_t *)val_data = sizeof(struct m0_cas_ctg);
		if (ctg != NULL)
			memcpy(val_data + sizeof(uint64_t), ctg,
				   ...
				   struct m0_be_tx_credit *accum)
{
	m0_be_btree_insert_credit2(bt, nr,
				   ...
				   sizeof(uint64_t) + sizeof(struct m0_cas_ctg),
				   accum);
}
...
M0_INTERNAL int m0_ctg_store_init()
{
	...
	if (result == 0) {
		...
		if (meta == NULL)
			...
			ctg_store.cs_meta = meta;
			ctg_init(meta, seg);

			/* Searching for catalogue-index catalogue. */
			result = ctg_meta_find_ctidx(meta, &ctidx);
			if (result == 0) {
	...
	ctg_store_init_creds_calc(seg, meta, ctidx, &cred);
	...
	result = ctg_meta_create(seg, &tx, &meta);
	if (result == 0) {
		/* Create catalog-index catalogue. */
		result = ctg_ctidx_create(seg, &tx, &ctidx);
		if (result == 0) {
			/*
			 * Insert catalogue-index catalogue into meta-catalogue.
			 */
			result = ctg_meta_insert(&meta->cc_tree,
						 &m0_cas_ctidx_fid,
						 ctidx,
						 &tx);
			if (result != 0)
		...
		if (result == 0) {
			...
			if (result == 0) {
				...
				/* Cleanup catalogue-index catalogue. */
				ctg_meta_delete(&meta->cc_tree,
						&m0_cas_ctidx_fid,
						&tx);
				ctg_ctidx_destroy(ctidx, seg, &tx);
				/* Cleanup meta-catalogue. */
				ctg_meta_destroy(meta, seg, &tx);
			}
		} else
			ctg_meta_destroy(meta, seg, &tx);
	}
...
static void ctg_store_release(struct m0_ref *ref)
{
	struct m0_ctg_store *ctg_store = container_of(ref,
						      struct m0_ctg_store,
						      cs_ref);
	ctg_store->cs_meta = NULL;
	ctg_store->cs_ctidx = NULL;
...
static bool ctg_op_cb(struct m0_clink *clink)
{
	...
	if (rc == 0) {
		...
		case CTG_OP_COMBINE(CO_PUT, CT_BTREE):
			*(uint64_t *)arena = ctg_op->co_val.b_nob;
			memcpy(arena + sizeof(uint64_t),
			       ctg_op->co_val.b_addr,
			       ctg_op->co_val.b_nob);
			break;
		case CTG_OP_COMBINE(CO_PUT, CT_META):
			*(uint64_t *)arena = sizeof(struct m0_cas_ctg);
			rc = ctg_create(arena + sizeof(uint64_t),
			 ...
			 int                  next_phase)
{
	...
	ctg_op->co_ctg = ctg_store.cs_meta;
...
M0_INTERNAL void m0_ctg_meta_cursor_init(struct m0_ctg_op *ctg_op)
{
	...
	m0_ctg_cursor_init(ctg_op, ctg_store.cs_meta);
}
...
M0_INTERNAL void m0_ctg_meta_insert_credit(struct m0_be_tx_credit *accum)
{
	m0_bcount_t         knob;
	m0_bcount_t         vnob;
	struct m0_be_btree *btree = &ctg_store.cs_meta->cc_tree;

	m0_be_btree_create_credit(btree, 1, accum);

	knob = sizeof(uint64_t) + sizeof(struct m0_fid);
	vnob = sizeof(uint64_t) + sizeof(struct m0_cas_ctg);
