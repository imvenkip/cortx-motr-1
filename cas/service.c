...
static int cas_fom_tick(struct m0_fom *fom0)
{
	...
	case CAS_META_LOOKUP_DONE:
		...
		if (rc == 0) {
			...
			m0_long_unlock(m0_ctg_lock(meta), &fom->cf_meta);
			M0_LOG(M0_ALWAYS,"vcp: crow: opc = %d, fid = "FID_F,opc, FID_P(&op->cg_id.ci_fid));
	...
	case CAS_CTIDX_INSERT:
		M0_LOG(M0_ALWAYS,"vcp: opc = %d, ct = %d,"FID_F,opc, ct, FID_P(&icid->ci_fid));
...
static bool cas_is_layout_ctg(struct m0_cas_op   *op)
{
	if(op)
	{
			if(m0_dix_fid_validate_cctg(&op->cg_id.ci_fid))
			{
				...
				{
					M0_LOG(M0_ALWAYS,"vcp: layout fid look up done");
					return true;
				}
			}
	}
		    ...
		    uint64_t rec_pos, int next)
{
	...
	if (ct == CT_META)
		 ...
		 */
		cid = NULL;
	M0_LOG(M0_ALWAYS,"vcp: ct = %d, opc = %d, cid = %p ", (int)ct, (int)opc, cid);
	if(cid)	
	 M0_LOG(M0_ALWAYS,"vcp: "FID_F, FID_P(&cid->ci_fid));
	if (opc != CO_CUR)
