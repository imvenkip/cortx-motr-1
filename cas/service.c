...
static bool m0_cas_fom_in_deadlock(const struct m0_fom *fom0)
{
	struct cas_fom         *fom   = M0_AMB(fom, fom0, cf_fom);;
	struct m0_long_lock    *lock  = m0_ctg_lock(fom->cf_ctg);
	const struct m0_be_tx  *tx    = &fom0->fo_tx.tx_betx;
...
M0_INTERNAL void m0_fom_in_deadlock(const struct m0_fom *fom0)
{
	...
	if (phase < M0_FOPH_TYPE_SPECIFIC && 
	    !M0_IN(fom0->fo_type->ft_id, (M0_CAS_GET_FOP_OPCODE,
	...
	if (m0_cas_fom_in_deadlock(fom0))
		exit(1);
}
