...
struct m0_fop_type repair_sw_onwire_rep_fopt;
extern struct m0_cm_type sns_repair_cmt;

static int repair_sw_fom_create(struct m0_fop *fop, struct m0_fom **m,
				struct m0_reqh *reqh)
{
	struct m0_fop *r_fop;
	int            rc;

	r_fop = m0_fop_reply_alloc(fop, &repair_sw_onwire_rep_fopt);
	if (r_fop == NULL)
...
M0_INTERNAL void m0_sns_cm_repair_sw_onwire_fop_fini(void)
{
	...
	m0_sns_cm_sw_onwire_fop_fini(&repair_sw_onwire_rep_fopt);
}
				     ...
				     const struct m0_cm_sw *out_interval)
{
	return m0_sns_cm_sw_onwire_fop_setup(cm, &repair_sw_onwire_fopt, fop,
					     fop_release, proxy_id, local_ep, sw, out_interval);
}
