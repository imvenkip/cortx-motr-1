...
rsink_allocation_failed:
	if (rsink == NULL)
		return M0_ERR(-ENOMEM, "m0_addb_mc_configure_rpc_sink()");
...
addb_ts_init_failed:
	if (rc != 0) {
		m0_free(rsink);
		return M0_ERR(rc, "m0_addb_mc_configure_rpc_sink()");
	}
					       ...
					       struct m0_rpc_conn *conn)
{
	...
	if (rpcsink_item_source == NULL)
		return M0_ERR(-ENOMEM, "m0_addb_mc_rpc_sink_source_add");

	rc = rpcsink_item_source_init(rpcsink_from_mc(mc), rpcsink_item_source);
	if (rc != 0) {
		m0_free(rpcsink_item_source);
		return M0_ERR(rc, "m0_addb_mc_rpc_sink_source_add");
	}
