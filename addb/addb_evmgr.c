				  ...
				  struct m0_addb_mc *dest_mc)
{
	...
	while (ts_rec != NULL) {
		...
		if (addb_rec == NULL) {
			...
			addb_ts_save(&cache->ace_ts, ts_rec);
			return M0_ERR_INFO(-ENOMEM, "Dest addb mc's rec alloc");
		}
						 ...
						 m0_bcount_t        pgsize)
{
	...
	M0_ALLOC_PTR(cache);
	if (cache == NULL)
		return M0_ERR_INFO(-ENOMEM, "Unable to allocate memory for "
				"cache evmgr");

	rc = addb_ts_init(&cache->ace_ts, npages, npages, pgsize);
	if (rc != 0) {
		m0_free(cache);
		return M0_ERR_INFO(rc, "Cannot initialize addb transient store");
	}
