...
/**
 * Indicates current idx needed by intercepted callback to verify data
 * incremented in cache flush
 */
static int ut_ts_rec_count;
static size_t addb_ut_mc_cache_rs_rec_len[50];
...
#undef CNTR_DATA_SIZE
	...
	/* synchronize posts only in case of pt evmgr */
...
static void addb_ut_evmgr_pt_cb(const struct m0_addb_rec *rec)
{
	...
	int i;
	size_t *reclen;
	int rec_is_addb_cntr = 0;

	if (addb_rec_post_ut_data[ut_ts_rec_count].brt > M0_ADDB_BRT_SEQ)
		return;

	/* We explicitly embedded a context in the thread argument so we
	 * can recover the thread argument structure here.
	 */
	ta = container_of(addb_rec_post_ut_data[ut_ts_rec_count].cv[0],
	...
	fields_nr = addb_rec_post_ut_data[ut_ts_rec_count].fields_nr;
	fields    = addb_rec_post_ut_data[ut_ts_rec_count].fields;

	switch (addb_rec_post_ut_data[ut_ts_rec_count].brt) {
	case M0_ADDB_BRT_EX:
	...
	/* validate */
	M0_UT_ASSERT(rec->ar_rid == addb_rec_post_ut_data[ut_ts_rec_count].rid);
	M0_UT_ASSERT(addb_rec_post_ut_data[ut_ts_rec_count].cv_nr == 3);
	...
	/*
	 * For caching evmgr. check for counter seq only as the counter is
	 * reset after posting and this filling of global ut_data array happens
	 * in cache flush
	 */
	if (rec_is_addb_cntr && ut_cache_evmgr_idx != 0)
		M0_UT_ASSERT(rec->ar_data.au64s_data[0] == fields[0]);
	else
		for (i = 0; i < fields_nr; ++i)
			M0_UT_ASSERT(rec->ar_data.au64s_data[i] == fields[i]);
	if (ut_cache_evmgr_idx == 0) {
		M0_UT_ASSERT(addb_ut_mc_rs_rec_len ==
			     addb_rec_post_ut_data[ut_ts_rec_count].reclen);
		*reclen = addb_ut_mc_rs_rec_len;
	} else {
		M0_UT_ASSERT(addb_ut_mc_cache_rs_rec_len[ut_ts_rec_count] ==
			     addb_rec_post_ut_data[ut_ts_rec_count].reclen);
		*reclen = addb_ut_mc_cache_rs_rec_len[ut_ts_rec_count];
	}


	++addb_ut_evmgr_pt_cb_num;

	addb_rec_post_ut_data[ut_ts_rec_count].brt = M0_ADDB_BRT_NR; /* reset */
}
...
static void addb_ut_evmgr_pt_cb2(const struct m0_addb_rec *rec)
{
	...
	if (addb_rec_post_ut_data[ut_ts_rec_count].brt != M0_ADDB_BRT_EX)
		return;
	...
	/* validate */
	M0_UT_ASSERT(rec->ar_rid == addb_rec_post_ut_data[ut_ts_rec_count].rid);
	M0_UT_ASSERT(addb_rec_post_ut_data[ut_ts_rec_count].cv_nr == cv_nr);
	...
	M0_UT_ASSERT(addb_rec_post_ut_data[ut_ts_rec_count].fields_nr ==
		     fields_nr);
	M0_UT_ASSERT(rec->ar_data.au64s_nr == fields_nr);
	for (i = 0; i < fields_nr; ++i)
		M0_UT_ASSERT(rec->ar_data.au64s_data[i] == fields[i]);
...
void addb_ut_reg_rec_types(void)
{
	/*
...
void addb_ut_validate_rec_len(void)
{
	int i;
...
void addb_ut_evmgr_cleanup(void)
{
	int i;

	if (ut_cache_evmgr_idx > 0) {
		for (i = 0; i < ut_ts_rec_count; ++i) {
			m0_free(addb_rec_post_ut_data[i].fields);
			m0_free(addb_rec_post_ut_data[i].cv);
		}
	}
	...
	for (i = 0; i < 10; ++i) {
		...
		M0_UT_ASSERT(m0_addb_rec_type_lookup(addb_ut_evmgr_pt_ta[i]
						     .ex->art_id) == NULL);
		addb_ut_evmgr_pt_ta[i].ex->art_magic = 0;
		addb_rt_tlist_del(addb_ut_evmgr_pt_ta[i].dp);
		M0_UT_ASSERT(m0_addb_rec_type_lookup(addb_ut_evmgr_pt_ta[i]
						     .dp->art_id) == NULL);
		addb_ut_evmgr_pt_ta[i].dp->art_magic = 0;
...
static void addb_ut_evmgr_pt_post_test(void)
{
	...
	m0_addb_mc_fini(&mc);
	addb_ut_evmgr_cleanup();
}

/*
 ****************************************************************************
 * Test to configure a caching event manager
 ****************************************************************************
 */
static void addb_ut_evmgr_cache_config_test(void)
{
	struct m0_addb_mc        mc = { 0 };
	struct m0_addb_mc_evmgr *evmgr;
	...
	addb_ut_mc_reset();
	m0_addb_mc_init(&mc);
	m0_addb_mc_configure_cache_evmgr(&mc, 1, 4096);

	evmgr = mc.am_evmgr;
	M0_UT_ASSERT(evmgr != NULL);
	M0_UT_ASSERT(evmgr->evm_magic == M0_ADDB_CACHE_EVMGR_MAGIC);
	M0_UT_ASSERT(evmgr->evm_put == addb_cache_evmgr_put);
	M0_UT_ASSERT(evmgr->evm_get == addb_cache_evmgr_get);
	M0_UT_ASSERT(evmgr->evm_rec_alloc == addb_cache_evmgr_rec_alloc);
	M0_UT_ASSERT(evmgr->evm_post == addb_cache_evmgr_rec_save);
	M0_UT_ASSERT(evmgr->evm_copy == addb_cache_evmgr_flush);
	M0_UT_ASSERT(evmgr->evm_post_awkward);

	cache = container_of(evmgr, struct addb_cache_evmgr, ace_evmgr);
	M0_UT_ASSERT(m0_atomic64_get(&cache->ace_ref.ref_cnt) == 1);

	/* fini */
	m0_addb_mc_fini(&mc);
}

static void addb_ut_evm_post(struct m0_addb_mc *mc, struct m0_addb_rec *rec)
{
	addb_ut_mc_rs_save(mc, rec);
	++ut_ts_rec_count;
}

static struct m0_addb_rec *addb_ut_cache_evm_rec_alloc(struct m0_addb_mc *mc,
						       size_t len)
{
	addb_ut_mc_cache_rs_rec_len[ut_cache_evmgr_idx] = len;
	++ut_cache_evmgr_idx;
	return addb_cache_evmgr_rec_alloc(mc, len);
}

static void addb_ut_evmgr_cache_post_test(void)
{
	struct m0_addb_mc   src_mc = { 0 };
	struct m0_addb_mc   dest_mc = { 0 };
	int                 i;
	int                 rc = 0;

	/* init */
	addb_ut_pt_evmgr = false;
	for (i = 0; i < 10; ++i)
		M0_SET0(&addb_ut_evmgr_pt_ta[i]);
	addb_ut_evmgr_pt_cb_num = 0;
	M0_UT_ASSERT(rc == 0);
	addb_ut_mc_reset();
	m0_addb_mc_init(&src_mc);
	m0_addb_mc_init(&dest_mc);
	addb_ut_mc_configure_recsink(&dest_mc);
	m0_addb_mc_configure_pt_evmgr(&dest_mc);
	dest_mc.am_evmgr->evm_post = addb_ut_evm_post;
	m0_addb_mc_configure_cache_evmgr(&src_mc, 10, 8192);
	src_mc.am_evmgr->evm_rec_alloc = addb_ut_cache_evm_rec_alloc;
	M0_UT_ASSERT(m0_addb_mc_is_fully_configured(&dest_mc));
	addb_ut_mc_rs_save_cb = addb_ut_evmgr_pt_cb; /* intercept saves */

	addb_ut_reg_rec_types();
	/*
	 * Test
	 * Post ADDB records on background threads
	 */
	for (i = 0; i < ARRAY_SIZE(addb_ut_evmgr_pt_ta); ++i) {
		struct addb_ut_evmgr_pt_thread_arg *ta =
			&addb_ut_evmgr_pt_ta[i];
	...
	M0_UT_ASSERT(addb_ut_evmgr_pt_cb_num == 0);

	addb_cache_evmgr_flush(&src_mc, &dest_mc);

	addb_ut_validate_rec_len();

	m0_addb_mc_fini(&src_mc);
	m0_addb_mc_fini(&dest_mc);
	addb_ut_evmgr_cleanup();
