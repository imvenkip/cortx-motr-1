...
parser_error:
	m0_yaml_parser_error_detect(&yctx->yc_parser);
	return -EINVAL;
}
 ...
 */
static yaml_node_t *yaml2db_scalar_locate(struct m0_yaml2db_ctx *yctx,
					  const char *value)
{
	bool	     scalar_found = false;
	yaml_node_t *node;

	M0_PRE(yaml2db_context_invariant(yctx));
	M0_PRE(value != NULL);
 ...
 */
static bool yaml2db_section_invariant(const struct m0_yaml2db_section *ysec)
{
	if (ysec == NULL)
		return false;
	...
	if (ysec->ys_section_type >= M0_YAML_TYPE_NR)
		return false;
				     ...
				     const struct m0_yaml2db_section *ysec)
{
	int cnt = 0;
        ...
        M0_PRE(yaml2db_section_invariant(ysec));
	M0_PRE(key != NULL);
 ...
 */
static bool validate_mandatory_keys(const struct m0_yaml2db_section *ysec,
...
/**
  ...
  @retval yaml_node_t pointer if successful, NULL otherwise
 */
static char *yaml2db_key_locate(struct m0_yaml2db_ctx *yctx,
				...
				const char *yaml_str)
{
	...
	M0_PRE(yaml2db_context_invariant(yctx));
	M0_PRE(node != NULL);
	M0_PRE(yaml_str != NULL);

	m0_yaml2db_mapping_for_each (yctx, node, pair, k_node, v_node) {
				     ...
				     const char *conf_param)
{
	...
	yaml_node_pair_t	*pair;
	struct m0_cfg_storage_device__key  key;
	struct m0_cfg_storage_device__val  val;
	...
	m0_yaml2db_sequence_for_each(yctx, node, item, s_node) {
		...
		M0_SET_ARR0(valid_key_status);
		mandatory_keys_present = false;

		ysec->ys_ops->so_key_populate((void *) &key, str);
		m0_yaml2db_mapping_for_each (yctx, s_node, pair,
					     k_node, v_node) {
		...
		m0_db_pair_setup(&db_pair, &table, &key, sizeof key,
				&val, sizeof val);
				     ...
				     const char *conf_param)
{
	...
	struct m0_cfg_storage_device__key  key;
	struct m0_cfg_storage_device__val  val;
	struct m0_cfg_storage_device__key  last_key;
	...
	m0_db_pair_setup(&db_pair, &table, &last_key, sizeof last_key, NULL, 0);
...
{
        struct m0_cfg_storage_device__key *dev_key = key;
...
#define DEV_ENUM_CHECK_INTERFACE(interface, val_str, dev_val) \
	if (strcmp(val_str, #interface) == 0) \
		dev_val->csd_type = M0_CFG_DEVICE_INTERFACE_ ## interface;

#define DEV_ENUM_CHECK_MEDIA(media, val_str, dev_val) \
	if (strcmp(val_str, #media) == 0)  \
		dev_val->csd_media = M0_CFG_DEVICE_MEDIA_ ## media;


/* Section op to populate the device value*/
static int device_val_populate (struct m0_yaml2db_section *ysec, void *val,
				const char *key_str, const char *val_str)
{
        struct m0_cfg_storage_device__val *dev_val = val;
...
/* Section ops */
const struct m0_yaml2db_section_ops m0_yaml2db_dev_section_ops = {
