...
enum {
	M0_RPC_SERVICE_TYPE_FOO = 1,
	M0_RPC_SERVICE_TYPE_BAR,
	M0_RPC_SERVICE_TYPE_NON_EXISTENT,
};
...
struct foo_service {
	struct m0_rpc_service f_service;
	int                   f_x;
};

static const struct m0_rpc_service_type_ops foo_service_type_ops = {
	.rsto_alloc_and_init = foo_service_alloc_and_init,
};

M0_RPC_SERVICE_TYPE_DEFINE(static, foo_service_type, "foo",
			   M0_RPC_SERVICE_TYPE_FOO, &foo_service_type_ops);

static const struct m0_rpc_service_ops foo_service_ops = {
			   ...
			   struct m0_rpc_service     **out)
{
	...
	M0_UT_ASSERT(service_type == &foo_service_type);
	foo_alloc_and_init_called = true;

	*out = NULL;

	M0_ALLOC_PTR(foo_service);
	M0_UT_ASSERT(foo_service != NULL);

	service = &foo_service->f_service;
	rc = m0_rpc__service_init(service, &foo_service_type, ep_addr,
...
static void foo_service_fini_and_free(struct m0_rpc_service *service)
{
	...
	M0_PRE(service != NULL && m0_rpc_service_bob_check(service));
...
M0_RPC_SERVICE_TYPE_DEFINE(static, bar_service_type, "bar",
			   M0_RPC_SERVICE_TYPE_BAR, &bar_type_ops);
...
{
	m0_rpc_service_type_register(&foo_service_type);
	M0_UT_ASSERT(m0_rpc_service_type_locate(M0_RPC_SERVICE_TYPE_FOO) ==
		     &foo_service_type);

	m0_rpc_service_type_register(&bar_service_type);
	M0_UT_ASSERT(m0_rpc_service_type_locate(M0_RPC_SERVICE_TYPE_BAR) ==
		     &bar_service_type);
}
...
{
	M0_UT_ASSERT(
		m0_rpc_service_type_locate(M0_RPC_SERVICE_TYPE_NON_EXISTENT) ==
			NULL
	);
}
...
static void alloc_test(void)
{
	...
	rc = m0_rpc_service_alloc_and_init(&foo_service_type, foo_ep_addr,
					   &foo_uuid, &fr_service);
	M0_UT_ASSERT(rc == 0);
	M0_UT_ASSERT(foo_alloc_and_init_called);
	M0_UT_ASSERT(fr_service != NULL);

	foo_service = container_of(fr_service, struct foo_service, f_service);
	M0_UT_ASSERT(foo_service->f_x == FOO_SERVICE_DEFAULT_X);
	M0_UT_ASSERT(strcmp(foo_ep_addr, m0_rpc_service_get_ep_addr(fr_service))
		     == 0);

	bar_alloc_and_init_called = false;
	rc = m0_rpc_service_alloc_and_init(&bar_service_type, NULL, NULL,
					   &service);
	M0_UT_ASSERT(bar_alloc_and_init_called);
	M0_UT_ASSERT(rc != 0 && service == NULL);
}
...
static void conn_attach_detach_test(void)
{
	...
	M0_SET0(&mock_conn);
	M0_SET0(&mock_rpc_chan);
	M0_SET0(&mock_destep);

	m0_sm_group_init(&mock_rpc_machine.rm_sm_grp);
	/* prepare mock rpc_machine */
	m0_rpc_services_tlist_init(&mock_rpc_machine.rm_services);

	/* prepare mock_destep */
	copy_of_foo_ep_addr = m0_alloc(strlen(foo_ep_addr) + 1);
	M0_UT_ASSERT(copy_of_foo_ep_addr != NULL);
	...
	mock_conn.c_sm.sm_state = M0_RPC_CONN_ACTIVE;
	mock_conn.c_rpc_machine = &mock_rpc_machine;
	mock_conn.c_rpcchan     = &mock_rpc_chan;

	m0_rpc_service_conn_attach(fr_service, &mock_conn);
	M0_UT_ASSERT(m0_rpc_service_invariant(fr_service));
	M0_UT_ASSERT(fr_service->svc_state ==
		     M0_RPC_SERVICE_STATE_CONN_ATTACHED);
	M0_UT_ASSERT(fr_service->svc_conn == &mock_conn);
	M0_UT_ASSERT(
		m0_rpc_services_tlist_head(&mock_rpc_machine.rm_services) ==
		     fr_service);

	m0_rpc_service_conn_detach(fr_service);
	M0_UT_ASSERT(m0_rpc_service_invariant(fr_service));
	M0_UT_ASSERT(fr_service->svc_state ==
		     M0_RPC_SERVICE_STATE_INITIALISED);
	M0_UT_ASSERT(
		m0_rpc_services_tlist_is_empty(&mock_rpc_machine.rm_services));


	m0_rpc_services_tlist_fini(&mock_rpc_machine.rm_services);

	m0_free(copy_of_foo_ep_addr);
	m0_sm_group_fini(&mock_rpc_machine.rm_sm_grp);
}

static void service_free_test(void)
{
	foo_service_fini_and_free_called = false;
	m0_rpc_service_fini_and_free(fr_service);
	fr_service = NULL;
	M0_UT_ASSERT(foo_service_fini_and_free_called);
}

static void unregister_test(void)
{
	m0_rpc_service_type_unregister(&foo_service_type);
	M0_UT_ASSERT(m0_rpc_service_type_locate(M0_RPC_SERVICE_TYPE_FOO) ==
		     NULL);

	m0_rpc_service_type_unregister(&bar_service_type);
	M0_UT_ASSERT(m0_rpc_service_type_locate(M0_RPC_SERVICE_TYPE_BAR) ==
		     NULL);
}

const struct m0_test_suite rpc_service_ut = {
