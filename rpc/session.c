		     ...
		     struct c2_rpcmachine	*machine)
{
	...
	if (rc != 0) {
		conn->c_state = CS_CONN_UNINITIALISED;
		return rc;
	}
	...
	C2_POST(ergo(rc == 0, conn->c_state == CS_CONN_INITIALISING &&
		c2_list_contains(&machine->cr_rpc_conn_list, &conn->c_link)));
	C2_POST(ergo(rc != 0, conn->c_state == CS_CONN_UNINITIALISED));
	return rc;
}
...
void c2_rpc_conn_terminate_reply_received(struct c2_fop *fop)
{
	...
	C2_POST(!c2_list_contains(&machine->cr_rpc_conn_list,
			&conn->c_link));
	C2_POST(conn->c_state == CS_CONN_TERMINATED ||
			conn->c_state == CS_CONN_FAILED);
...
out:
	...
	C2_POST(ergo(rc == 0, session->s_state == SESSION_CREATING &&
				       ...
				       struct c2_rpc_item	**out)
{
	...
	*out = NULL;
	...
	c2_mutex_lock(&item->ri_mach->cr_session_mutex);
	conn_search(item->ri_mach, item->ri_sender_id, &conn);
	if (conn == NULL) {
			   ...
			   struct c2_db_tx	*tx)
{
	...
	*out = NULL;
	 ...
	 */
	rc = c2_rpc_cob_lookup_helper(dom, NULL, "SESSIONS", &root_session_cob,
						tx);
	if (rc != 0)
		return rc;
