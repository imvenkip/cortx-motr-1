/* -*- C -*- */
#ifndef _SESSION_PROTO_H_

#define _SESSION_PROTO_H_
...
struct session_create_arg {
	struct client_id sca_client;
	struct client_id sca_server;
	uint32_t sca_high_slot_id;
	uint32_t sca_max_rpc_size;
};

struct session_create_out {
	struct session_id sco_session_id;
...
struct c2_session_adjust_out {
	int errno;
	union {
		struct c2_session_adjust_rep session_adjust_reply;
	} c2_session_adjust_out_u;
};
