...
static void create_send()
{
	...
	for (i = 0; i < 10; ++i) {
		...
		item->ri_group = NULL;
		item->ri_type = &c2_stob_create_rpc_item_type;
		ftype = fop->f_type;
		ftype->ft_ri_type = &c2_stob_create_rpc_item_type;
...
static void read_send()
{
	...
	for (i = 0; i < 10; ++i) {
		...
		item->ri_prio = C2_RPC_ITEM_PRIO_MAX;
		item->ri_group = NULL;
		item->ri_type = &c2_stob_read_rpc_item_type;
		ftype = fop->f_type;
		ftype->ft_ri_type = &c2_stob_read_rpc_item_type;
		item->ri_session = &cl_rpc_session;
		c2_time_set(&timeout, 60, 0);
		c2_clink_init(&clink, NULL);
		c2_clink_add(&item->ri_chan, &clink);
...
static void write_send()
{
	...
	for (i = 0; i < 10; ++i) {
		...
		item->ri_prio = C2_RPC_ITEM_PRIO_MAX;
		item->ri_group = NULL;
		item->ri_type = &c2_stob_write_rpc_item_type;
		ftype = fop->f_type;
		ftype->ft_ri_type = &c2_stob_write_rpc_item_type;
		item->ri_session = &cl_rpc_session;
		c2_time_set(&timeout, 60, 0);
		c2_clink_init(&clink, NULL);
		c2_clink_add(&item->ri_chan, &clink);
