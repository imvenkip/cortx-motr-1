...
static void fom_rep_cb(struct c2_clink *clink)
{
        ...
        if (clink != NULL) {
                struct c2_fop *rfop= c2_alloc(sizeof *rfop);/*container_of(clink, struct c2_fop, rep_clink);*/
                if (rfop != NULL) {
			printf("\n reply received \n");
...
********************************************************************/
static void create_send(const struct c2_fom_fop_fid *fid)
{
        struct c2_fop                    *f;
        struct c2_fop                    *r;
	...
	c2_reqh_fop_handle(&reqh, f, r);
}

static void read_send(const struct c2_fom_fop_fid *fid)
{
        struct c2_fop                    *f;
        struct c2_fop                    *r;
...
static void write_send(const struct c2_fom_fop_fid *fid)
{
        struct c2_fop                    *f;
        struct c2_fop                    *r;
...
static void reqh_test_send(unsigned long seq, unsigned long oid)
{	
...
int read_fom_state(struct c2_fom *fom)
{
	...
	/** Will be using non-blocking c2_rpc_reply_submit() in future **/
	printf("GOT: %i %i\n", out_fop->sirr_rc, out_fop->sirr_buf.cib_count);
	printf("\t[");
	for (j = 0; j < out_fop->sirr_buf.cib_count; ++j)
		printf("%02x", out_fop->sirr_buf.cib_value[j]);
	printf("]\n");
	++reply;
        return FSO_AGAIN;
}
...
int write_fom_state(struct c2_fom *fom)
{
	...
	printf("GOT: %i %i\n", out_fop->siwr_rc, out_fop->siwr_count);
	++reply; 
        return FSO_AGAIN;
}
...
{
        c2_fop_object_fini();
        c2_fop_type_fini_nr(fopt, ARRAY_SIZE(fopt));
...
int fom_io_fop_init(void)
{
        ...
        if (result == 0) {
                result = c2_fop_type_build_nr(fopt, ARRAY_SIZE(fopt));
                if (result == 0)
...
void test_reqh(void)
{
	...
	result = fom_io_fop_init();
        ...
        result =  c2_reqh_init(&reqh, NULL, NULL, sdom, &fol, &service);

	/* create listening thread to accept async reply's */
	
	c2_chan_init(&rep_chan);
	c2_clink_init(&rclink, &fom_rep_cb);
	...
	c2_clink_fini(&rclink);
	c2_chan_fini(&rep_chan);
        c2_reqh_fini(&reqh);
