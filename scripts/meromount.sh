...
insmod build_kernel_modules/kmero.ko local_addr=$MEP \
...
for ((i=0; i < ${#EP[*]}; i++)) ; do
	...
	(cd $WORK_ARENA/d$i
	 $HERE/mero/mero_setup -r -p -T ${STOB_TYPE}         \
...
# for starting up a server, so wait till all mero_setup services are started.
...
mount -t m0t1fs -o "$CONF,$IOS" none $MP
mount | grep m0t1fs
